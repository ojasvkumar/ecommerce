...
const ProductListItems = ({product}) => {
	...
	const {price, shipping, brand, color, category, subs, quantity, sold} = product;
	    ...
	    <ul className="list-group">
		...
		
		    {
		    	subs && (
		    		<li className="list-group-item">
					    Sub Categories
					    {subs.map(s => <Link key={s._id} to={`/subs/${s.slug}`} className="label label-default label-pill pull-xs-right">
						    {s.name}
					    </Link>)}
				    </li>
			    )
		    ...
		    <li className={`list-group-item ${quantity < 1 && `text-danger`}`}>
